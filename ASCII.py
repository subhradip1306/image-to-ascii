...
def square_to_border(image):
    ...
    border_list = [].append(assign_border([pixels[i*2], pixels[1+i*2], pixels[image.getdata()[0]/2+i*2], pixels[1+image.getdata()[0]/2+i*2]]) for i in range(image.size[0]*image.size[1]//4))
...
def image_to_ascii(image_path, output_width=149):
    ...
    for i in range(len(ascii_list)):
        if border_list[i] == None:
            ascii_str = ascii_str.join(ascii_list[i])
        else:
            ascii_str = ascii_str.join(border_list[i])
